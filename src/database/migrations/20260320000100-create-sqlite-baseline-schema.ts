...
import { getMigrationDatabaseType } from '../migration-database.util';
...
export class CreateSqliteBaselineSchema20260320000100
  implements MigrationInterface
{
  name = 'CreateSqliteBaselineSchema20260320000100';

  public async up(queryRunner: QueryRunner): Promise<void> {
    if (getMigrationDatabaseType(queryRunner) !== 'sqlite') {
      return;
    }

    const existingTables = await Promise.all(
      SQLITE_BASELINE_TABLES.map((tableName) => queryRunner.hasTable(tableName)),
    );
    ...
    if (existingCount > 0) {
      throw new Error(
        'Se detecto un schema SQLite parcial. Usa una base nueva para inicializar el baseline local.',
      );
    }
