...
export class CreateSyncOutboxEvents20260329000100
  implements MigrationInterface
{
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    const nowExpression =
      databaseType === 'sqlite' ? "datetime('now')" : 'CURRENT_TIMESTAMP';
    ...
    const payloadDefault =
      databaseType === 'sqlite' ? "DEFAULT ('{}')" : `DEFAULT '{}'::jsonb`;
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (getMigrationDatabaseType(queryRunner) === 'postgres') {
      await queryRunner.query(`
        DO $$
        BEGIN
          IF EXISTS (
