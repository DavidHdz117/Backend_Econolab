...
async function main() {
  ...
  const { Doctor } = await import('../src/doctors/entities/doctor.entity');
  const { Patient } = await import('../src/patients/entities/patient.entity');
  const {
    ServiceOrder,
    ServiceOrderItem,
    ServiceItemPriceType,
    ServiceStatus,
  } = await import('../src/services/entities/service-order.entity');
  const { SyncOutboxEvent, SyncOutboxOperation } = await import(
    '../src/sync/entities/sync-outbox-event.entity'
  );
  const { Study } = await import('../src/studies/entities/study.entity');
  const { StudyDetail } = await import('../src/studies/entities/study-detail.entity');
  const { StudyResult, StudyResultValue } = await import(
    '../src/results/entities/study-result.entity'
  );
  ...
  try {
    ...
    const doctorPublicId = randomUUID();
    const studyPublicId = randomUUID();
    const detailPublicId = randomUUID();
    const servicePublicId = randomUUID();
    const itemPublicId = randomUUID();
    const resultPublicId = randomUUID();
    const valuePublicId = randomUUID();
