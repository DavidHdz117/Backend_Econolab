...
import { getAppRuntimeConfig } from '../src/config/app.config';
import { getDatabaseRuntimeConfig } from '../src/config/database.config';
import { getIntegrationRuntimeConfig } from '../src/config/integrations.config';
import { getStorageRuntimeConfig } from '../src/config/storage.config';
import { getSyncRuntimeConfig } from '../src/config/sync.config';
import { buildRuntimeDiagnostics } from '../src/runtime/runtime-diagnostics.util';
...
function createEnv(overrides: Record<string, string | undefined>) {
  return {
    ...
    DATABASE_SQLITE_PATH:
      overrides.DATABASE_SQLITE_PATH ??
      resolve(process.cwd(), 'tmp', `${overrides.APP_RUNTIME_MODE ?? 'web'}.sqlite`),
...
function getDiagnosticsForEnv(env: NodeJS.ProcessEnv) {
  const app = getAppRuntimeConfig(env);
  const database = getDatabaseRuntimeConfig(env);
  const integrations = getIntegrationRuntimeConfig(env);
  const storage = getStorageRuntimeConfig(env);
  const sync = getSyncRuntimeConfig(env);
...
async function main() {
  ...
  assert(
    webOnline.storage.profileImageStorageMode === 'database',
    'web-online debe seguir usando imagen de perfil en DB por compatibilidad.',
  );
  ...
  assert(
    desktopOnline.status === 'ready',
    'desktop-online sano debio quedar ready.',
  );
  assert(
    desktopOnline.sync.outboxEnabled === true,
    'desktop-online debe dejar outbox activado por default.',
  );
  assert(
    desktopOnline.storage.profileImageStorageMode === 'filesystem',
    'desktop-online debe preferir filesystem para imagenes de perfil.',
  );
  ...
  assert(
    desktopOffline.status === 'ready',
    'desktop-offline sano debio quedar ready.',
  );
  assert(
    desktopOffline.integrations.mailEnabled === false &&
      desktopOffline.integrations.googleAuthEnabled === false &&
      desktopOffline.integrations.gmailOauthEnabled === false,
    'desktop-offline debe desactivar integraciones online por default.',
  );
  assert(
    desktopOffline.sync.outboxEnabled === true,
    'desktop-offline debe dejar outbox activado por default.',
  );
  ...
  assert(
    riskyDesktopOffline.status === 'blocked',
    'desktop-offline riesgoso debio quedar blocked.',
  );

  const riskyIssueCodes = new Set(
    riskyDesktopOffline.issues.map((issue) => issue.code),
  );
  for (const requiredCode of [
    ...
    assert(
      riskyIssueCodes.has(requiredCode),
      `Falto el warning esperado ${requiredCode} en desktop-offline riesgoso.`,
    );
  }
